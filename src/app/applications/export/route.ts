 ...
 * GET() when the browser requests /applications/export.
 */
export async function GET() {
  ...
  if (!user) {
    return new Response("Sign in to export your applications.", { status: 401 });
  }
  ...
  const filename = `jobtrail-applications-${new Date().toISOString().slice(0, 10)}.csv`;
