...
export function MatchResult({ outcome, jobId }: { outcome: MatchOutcome; jobId: string }) {
  if (outcome.status === "no-resume") {
    return (
      <p className="border border-gray-300 rounded p-4 text-sm">
        No match score:{" "}
  ...
  if (outcome.status === "unavailable") {
    return (
      <p className="border border-gray-300 rounded p-4 text-sm text-gray-500">
    ...
    <div className="border border-gray-300 rounded p-4 space-y-2">
      ...
      {missing.length > 0 && (
        <div className="text-sm">
          <p className="text-gray-500 mb-1">Must-haves your resume doesn&apos;t show:</p>
      ...
      <Link href={`/match/${jobId}`} className="text-sm text-blue-600 hover:underline inline-block">
        Full breakdown →
      </Link>
    </div>
  );
